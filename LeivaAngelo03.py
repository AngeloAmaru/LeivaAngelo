#Lin GitHub https://github.com/AngeloAmaru/LeivaPrueba3.git
import csv
list=[]
def confirm():      #Terminado
    ...
    if confirm==True:
        list.remove(x)
def stats(): # No terminado /// Las estadísticas contemplan imprimir los siguientes datos : ///1.- Porcentaje de efectividad promedio ///2.- Valor del porcentaje de efectividad más alto
    peak=0
    total=0
    for x in list:              #NoTerminado
        porcentaje=int((x[2]))    
    total=total+porcentaje
    lamount=list.count(x[2])
    average=total/lamount
    print("Promedio: ",average)
    print("Porcentaje más Alto ", peak)
def newcsv():               #No Terminado
    with open("plan.csv","w",newline="") as archivo:
        writecsv=archivo.writable
        writecsv

def importcsv():            #No terminado
    list.clear
    with open("plan.csv","r",newline="") as archivo:
        csvread=archivo.read
